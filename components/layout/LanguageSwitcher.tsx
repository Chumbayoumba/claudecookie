...
/** Removes a leading `/ru` or `/zh` so the path can be re-prefixed for another locale. */
export function stripLocale(pathname: string): string {
  const [, first, ...rest] = pathname.split('/')
  if (first && isLocale(first) && first !== 'en') {
    return `/${rest.join('/')}`
  }
  return pathname
}
...
export function LanguageSwitcher({ locale, label }: LanguageSwitcherProps) {
  ...
  function choose(next: Locale) {
    ...
    // A full navigation, for the same reason the links are plain anchors.
    window.location.assign(localePath(next, stripLocale(pathname)))
  }
          ...
          {LOCALES.map((l) => (
            <button
              ...
              className={cn(
                ...
                'font-sans text-detail-s transition-colors duration-150 ease-ant',
